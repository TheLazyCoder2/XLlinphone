...
static bool linphoneVideoOpen = true;//自己加的  拨打接收视频开启
...
//	const LinphoneAddress *addr = linphone_call_get_remote_address(call);
	NSString *address = @"123456";//[FastAddressBook displayNameForAddress:addr];
...
- (void)onConfiguringStatusChanged:(LinphoneConfiguringState)status withMessage:(const char *)message {
	NSLog(@"onConfiguringStatusChanged: %s %@", linphone_configuring_state_to_string(status), message ? [NSString stringWithFormat:@"(message: %s)", message] : @"");
...
//	}
	...
	if (![LinphoneManager isNotIphone3G]) {
		...
		if (pt) {
			...
			NSLog(@"SILK/24000 and video disabled on old iPhone 3G");
		}
...
//	[NSNotificationCenter.defaultCenter addObserver:self selector:@selector(inappReady:) name:kIAPReady object:nil];
    ...
    linphone_core_enable_ipv6(theLinphoneCore, YES);/***************启动IPV6*****************/
...
- (void)playMessageSound {
	...
	if (!success) {
		NSLog(@"Could not play the message sound");
	}
...
- (void)startCallPausedLongRunningTask {
    ...
    NSLog(@"Long running task started, remaining [%g s] because at least one call is paused", [[UIApplication sharedApplication] backgroundTimeRemaining]);
}
...
- (void)startPushLongRunningTask:(BOOL)msg {
	...
	NSLog(@"Long running task started, remaining [%g s] because a push has been received", [[UIApplication sharedApplication] backgroundTimeRemaining]);
}
...
- (void)renameDefaultSettings {
	...
	if ([fileManager fileExistsAtPath:src]) {
		if ([fileManager fileExistsAtPath:dst]) {
			...
			NSLog(@"%@ already exists, simply removing %@ %@", dst, src, fileError ? fileError.localizedDescription : @"successfully");
		} else {
...
//    linphone_core_enable_video_capture(theLinphoneCore, true);
    ...
    //视频开启 自己加的
    linphone_call_params_enable_video(lcallParams, linphoneVideoOpen);
...
#define APPMODE_SUFFIX @"prod"
		...
		NSLog(@"Proxy config %s NOT configured for push notifications", linphone_proxy_config_get_identity(proxyCfg));
		// no push token:
